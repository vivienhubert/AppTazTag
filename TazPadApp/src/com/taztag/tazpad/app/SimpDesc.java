...
public class SimpDesc implements ConfListener, ZclListener, NetworkListener {
	...
	private AndroidNDK1SampleActivity parent;
	private PassThroughClient measurement_temp;
	private int hd = -1;
	TAddress device_addr;
	
	public SimpDesc(AndroidNDK1SampleActivity _parent) {
	...
	public void onCommand(Object o, int clusterId, int command) {
		...
		if (srcAddrMode == 2) {
			...
			Log.d("TT_ZigBee_Sample", "[SimpDesc] Error="
					+ "Zigbee bad address");
			return;
		}
		int size=0;
		...
		parent.updateMyValue(value);
		Log.d("ZigBee", "[SimpDesc] ="
				+ "finished to receive data");
		
		Log.d("TT_ZigBee_Sample", "[SimpDesc] Zigbee=" + srcAddrMode + ";"
				+ srcAddrString + ";" + new String(data));
		parent.update(Notif.DATA_RESPONSE, new String(data));
	...
	public void networkInd(int type, Object ind) {
		...
		case NetworkListener.CHILD_JOINED:
			...
			Log.d("TT_ZigBee_Sample", "[SimpDesc] Child joined: ea="
					+ ind0.getExtAdd().toHexString() + " - sa="
					+ ind0.getShortAdd().toHexString() + " - capability="
					+ ind0.getCapability());
			...
			readFromTarget();
			parent.update(Notif.CONNECTED, 0);
			break;
		case NetworkListener.CHILD_REMOVED:
			...
			Log.d("TT_ZigBee_Sample",
					"[SimpDesc] Child removed: ea="
							+ ind1.getExtAdd().toHexString() + " - sa="
							+ ind1.getShortAdd().toHexString() + " addr = "
							+ nf.getAddr());
			if (ind1.getExtAdd().toHexString()
				...
				nf.setCanSend(false);
				nf.disjoined();
				parent.update(Notif.DISCONNECTED, 0);
		...
		case NetworkListener.NETWORK_LEFT:
			...
			Log.d("TT_ZigBee_Sample",
					"[SimpDesc] Network left(" + Integer.toHexString(type)
							+ ") - parentsa="
							+ ind2.getParentShortAdd().toHexString()
							+ " - panid=" + ind2.getPanId().toHexString()
							+ " - channel=" + ind2.getCurrentChannel()
							+ " - sa=" + ind2.getShortAdd().toHexString());
			break;
		case NetworkListener.NETWORK_LOST:
			Log.d("TT_ZigBee_Sample", "[SimpDesc] NETWORK LOST");
			NetworkUpdate ind3 = (NetworkUpdate) ind;
			Log.d("TT_ZigBee_Sample",
					"[SimpDesc] Network lost(" + Integer.toHexString(type)
							+ ") - parentsa="
							+ ind3.getParentShortAdd().toHexString()
							+ " - panid=" + ind3.getPanId().toHexString()
							+ " - channel=" + ind3.getCurrentChannel()
							+ " - sa=" + ind3.getShortAdd().toHexString());
			break;
		case NetworkListener.NETWORK_STARTED:
			Log.d("TT_ZigBee_Sample", "[SimpDesc] NETWORK STARTED");
			NetworkUpdate ind4 = (NetworkUpdate) ind;
			Log.d("TT_ZigBee_Sample",
					"[SimpDesc] Network started(" + Integer.toHexString(type)
							+ ") - parentsa="
							+ ind4.getParentShortAdd().toHexString()
							+ " - panid=" + ind4.getPanId().toHexString()
							+ " - channel=" + ind4.getCurrentChannel()
							+ " - sa=" + ind4.getShortAdd().toHexString());
			break;
		}
	}
	...
	public void apsConfirmation(int type, int handle, Object ob) {
		...
		case ConfListener.APS_REGISTER_EP_CNF:
			Log.d("TT_ZigBee_Sample", "[SimpDesc] APS_REGISTER_EP_CNF hd = "
					+ nf.getHandle() + " - handle = " + handle);

			if (nf.getHandle() == handle) {
				Log.d("TT_ZigBee_Sample",
						"[SimpDesc] APS_REGISTER_EP_CNF hd was equals to handle ; now hd = "
								+ hd);
				parent.update(Notif.STARTED, 0);
			}

			break;
		case ConfListener.APS_UNREGISTER_EP_CNF:
			Log.d("TT_ZigBee_Sample", "[SimpDesc] APS_UNREGISTER_EP_CNF // "
					+ "unregisterEndpointConf handle = " + handle + " - res = "
					+ ((Integer) ob).intValue());
			break;
		case ConfListener.APS_DATA_CNF:
			Log.d("TT_ZigBee_Sample", "[SimpDesc] APS_DATA_CNF + handle = "
					+ handle + " - +hd = " + nf.getHandle() + " - status = "
					+ ((DataConf) ob).getStatus());
			nf.setCanSend(true);
	...
	public void zdoConfirmation(int type, int handle, Object ob) {
		...
		case ConfListener.ZDO_STARTNETWORK_CNF:
			Log.d("TT_ZigBee_Sample",
					"[SimpDesc] ZDO_STARTNETWORK_CNF 0 ; hd = "
							+ nf.getHandle() + " ; handle = " + handle);
			int res = ((Integer) ob).intValue();
			Log.d("TT_ZigBee_Sample",
					"[SimpDesc] ZDO_STARTNETWORK_CNF - test = " + res);
			nf.setCanSend(false);
			if (res == 0) {
				nf.setHandle(sDesc.registerEndpoint());
				Log.d("TT_ZigBee_Sample",
						"[SimpDesc] ZDO_STARTNETWORK_CNF 1 ; hd = "
								+ nf.getHandle() + " ; handle = " + handle);
			} else {
				Log.d("TT_ZigBee_Sample", "[SimpDesc] startNetwork fails("
						+ Integer.toHexString(res) + ")");
				nf.setCanSend(false);
				parent.update(Notif.RESTART_ZIGBEE, 0);
			}
			break;
		case ConfListener.ZDO_MGMTLEAVE_CNF:
			Log.d("TT_ZigBee_Sample", "[SimpDesc] ZDO_MGMTLEAVE_CNF ; hd = "
					+ nf.getHandle() + " ; handle = " + handle);
			...
			if (res2 == 0) {
				...
				Log.d("TT_ZigBee_Sample", "[SimpDesc] mgmtLeave fails("
						+ Integer.toHexString(res2) + ")");
			}
			break;
		case ConfListener.ZDO_RESET_CNF:
			Log.d("TT_ZigBee_Sample", "[SimpDesc] resetConf handle " + handle
	...
	public void zdoResponse(int type, int handle, Object ob) {
		Log.d("TT_ZigBee_Sample", "[SimpDesc] ZDO_NWKADDR_RSP - handle = "
				+ handle + " - hd = " + nf.getHandle() + " - addr = "
				+ ((NwkAddrRsp) ob).getNwkAddrRemoteDev().toHexString());
		switch (type) {
		case ConfListener.ZDO_NWKADDR_RSP:
			Log.d("TT_ZigBee_Sample",
					"[SimpDesc] ZDO_NWKADDR_RSP : " + nf.getHandle()
			...
			if (nf.getAddr16() != null
				...
				nf.setCanSend(true);
				Log.d("TT_ZigBee_Sample",
						"[SimpDesc] ZDO_NWKADDR_RSP SWITCH CONNECTED");
				parent.update(Notif.CONNECTED, 0);
			}
			break;
		default:
